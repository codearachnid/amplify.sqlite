...
amplify.sqlite = function() {
	var self = this,
		environment = null,
		userConfiguration = typeof amplify.sqlite.configuration == 'object' ?
		amplify.sqlite.configuration : {};
	var config = $.extend({
		expiration: Date.now(),
		db: {
	...
	var sql = {
		...
		FLUSH: " DELETE FROM %(table.store)s ",
		VACUUM: " VACUUM ",
	...
	self.put = function(key, data, expiration) {
		...
		if (isEmpty(key) || isEmpty(data)) {
			error.message =
				'Key and data must be supplied to retrieve results.';
			error.code = 400;
			deferred.reject(error);
		}
	...
	self.delete = function(key) {
		...
		if (isEmpty(key)) {
			error.message =
				'Key must be supplied to delete stored items.';
			error.code = 400;
			deferred.reject(error);
		}
		var options = $.extend({}, config, {
		...
		execute(self.sprintf(sql.DELETE, options)).done(function(result,
			sql) {
	...
	self.clear = function() {
		var deferred = $.Deferred();
		execute(self.sprintf(sql.FLUSH, config)).done(function() {
			deferred.resolve = vacuum();
		}).fail(function(error) {
			deferred.reject(error);
		});
		return deferred.promise();
	...
	function clean() {
		...
		execute(self.sprintf(sql.EXPIRED, config)).done(function(
			request) {
	...
	function vacuum() {
		var deferred = $.Deferred();
		execute(self.sprintf(sql.VACUUM, config)).done(function(request) {
			deferred.resolve(request);
		});
		return deferred.promise();
	}
	...
	function execute(executeSQL) {
		...
		if (environment == null && typeof window.openDatabase ==
			'function') {
			environment = window.openDatabase(config.db.name, config.db
				.version, config.db.display, config.db.size,
				initEnvironment);
			initEnvironment();
		}
		if (environment !== null) {
			environment.transaction(function(tx) {
				tx.executeSql(executeSQL, [], function(tx,
					results) {
	...
	self.sprintf = function() {
		if (!self.sprintf.cache.hasOwnProperty(arguments[0])) {
			self.sprintf.cache[arguments[0]] = self.sprintf.parse(
				arguments[0]);
		}
		return self.sprintf.format.call(null, self.sprintf.cache[
		...
		for (i = 0; i < tree_length; i++) {
			...
			if (node_type === 'string') {
				...
				if (match[2]) { // keyword argument
					...
					for (k = 0; k < match[2].length; k++) {
						if (!arg.hasOwnProperty(match[2][k])) {
							throw (self.sprintf(
								'utils.sprintf property "%s" does not exist',
								match[2][k]));
						}
						arg = arg[match[2][k]];
					}
				...
				if (/[^s]/.test(match[8]) && (getType(arg) != 'number')) {
					throw (self.sprintf(
						'utils.sprintf expecting number but found %s',
						getType(arg)));
				}
				switch (match[8]) {
					case 'b':
					...
					case 'd':
						...
						break;
					case 'e':
						arg = match[7] ? arg.toExponential(match[7]) :
							arg.toExponential();
						break;
					case 'f':
						arg = match[7] ? parseFloat(arg).toFixed(match[
							7]) : parseFloat(arg);
						break;
					case 'o':
						arg = arg.toString(8);
						break;
					case 's':
						arg = ((arg = String(arg)) && match[7] ? arg.substring(
							0, match[7]) : arg);
						break;
					case 'u':
						arg = arg >>> 0;
	...
	self.sprintf.parse = function(fmt) {
		...
		while (_fmt) {
			if ((match = /^[^\x25]+/.exec(_fmt)) !== null) {
					...
					if ((field_match = /^([a-z_][a-z_\d]*)/i.exec(
						...
						while ((replacement_field = replacement_field.substring(
							...
							if ((field_match = /^\.([a-z_][a-z_\d]*)/i.exec(
								replacement_field)) !== null) {
								field_list.push(field_match[1]);
							} else if ((field_match = /^\[(\d+)\]/.exec(
								replacement_field)) !== null) {
								field_list.push(field_match[1]);
							} else {
								throw (
									'utils.sprintf incorrectly matched or no targets found.'
								);
							}
						}
					} else {
				...
				if (arg_names === 3) {
					throw (
						'utils.sprintf incorrectly mixing positional and named placeholders is not (yet) supported.'
					);
				}
